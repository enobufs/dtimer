...
DTimer.prototype.post = function (ev, delay, cb) {
    ...
    this._callOnReady(function (err) {
        if (err) { return void(cb(err)); }
        self._post(evId, msg, delay, cb);
    });
};

DTimer.prototype._post = function (evId, msg, delay, cb) {
    var self = this;
    this._redisTime(self._pub, function (err, now) {
        if (err) { return void(cb(err)); }
        self._pub.multi()
...
DTimer.prototype.confirm = function (evId, cb) {
    ...
    this._callOnReady(function (err) {
        if (err) { return void(cb(err)); }
        self._confirm(evId, cb);
    });
};

DTimer.prototype._confirm = function (evId, cb) {
    var self = this;
    this._redisTime(self._pub, function (err, now) {
        if (err) { return void(cb(err)); }
        self._pub.multi()
...
DTimer.prototype.changeDelay = function (evId, delay, cb) {
    ...
    this._callOnReady(function (err) {
        if (err) { return void(cb(err)); }
        self._changeDelay(evId, delay, cb);
    });
};

DTimer.prototype._changeDelay = function (evId, delay, cb) {
    var self = this;
    this._redisTime(self._pub, function (err, now) {
        if (err) { return void(cb(err)); }
        self._pub.multi()
