...
DTimer.prototype.post = function (ev, delay, opts, cb) {
    var self = this;
    var evId;
    ...
    var updateTime = opts.updateTime == null ? true : opts.updateTime;
    var updateMessage = opts.updateMessage == null ? true : opts.updateMessage;
    ...
    .then(function (now) {
        ...
        if (!updateTime) {
            zaddArgs.splice(1, 0, 'NX');
        }
        var multi = self._pub.multi()
        return multi
        .zadd.apply(multi, zaddArgs)
        [updateMessage ? 'hset' : 'hsetnx'](self._keys.ed, evId, msg)
        .hsetnx(self._keys.ed, evId, msg)
        .evalsha(
            scripts.update.sha,
            5,
            self._keys.gl,
