...
function DTimer(id, pub, sub, option) {
    ...
    this._lur.on('state', function(from, to) {
        ...
        if (to === 3) {
            if (self._readyCbTimer) {
                clearTimeout(self._readyCbTimer);
...
DTimer.prototype.upcoming = function(cb) {
    var
        upcomingMulti = this._pub.multi(),
        _self = this;

    this._pub.zrangebyscore(
        [_self._keys.ei,
        new Date().getTime(),
        '+inf',
        'WITHSCORES'],
        function(err,results){
            ...
            if (err) {
               ...
               for (i = 0; i < results.length; i += 2) {
                    out.push({ ts : results[i+1], id : results[i]});
                    upcomingMulti.hget(_self._keys.ed,results[i])
                }
                
                upcomingMulti.exec(function(err,data){
                    var
                        outObj = {};
                    if (err) {
                        cb(err);
                        ...
                        data.forEach(function(data,index){
                            if (data) {
                                outObj[out[index].ts] = { id : out[index].id, data: JSON.parse(data) };
                            }
                        });
                        cb(err,data.length === 0 ? false : outObj);
                    }
                });
            }
        }
